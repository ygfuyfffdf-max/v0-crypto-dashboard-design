...
async function closeModal(page: Page) {
  const closeBtn = page
    .locator(
      '[role="dialog"] button:has-text("×"), [role="dialog"] button:has-text("Cerrar"), [role="dialog"] button[aria-label*="close"], [role="dialog"] button[aria-label*="Close"]'
    )
    .first()
...
test.describe("♿ Accesibilidad", () => {
  test("debe tener estructura semántica", async ({ page }) => {
    ...
    console.log(
      `📝 Estructura: ${headingCount} headings, ${buttonCount} buttons, ${inputCount} inputs`
    )
  })
