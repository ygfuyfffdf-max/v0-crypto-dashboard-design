 ...
 * utilizando la página de demostración /chronos-ultra-enhanced
...
// ============================================
// TESTS EN DEMO (Sin Auth)
// ============================================

test.describe("🎪 Components Demo - Sin Auth", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/chronos-ultra-enhanced")
    await page.waitForLoadState("networkidle")
    await page.waitForTimeout(1000)
  })

  test("debe cargar la página de demo", async ({ page }) => {
    const currentUrl = page.url()

    // Si no redirige a login, estamos en demo
    if (!currentUrl.includes("login")) {
      console.log("✅ Demo cargado sin auth")

      // Tomar screenshot
      await page.screenshot({ path: "test-results/demo-loaded.png", fullPage: true })
...
test.describe("🎨 Componentes UI en Login", () => {
  ...
  test("debe mostrar/ocultar password", async ({ page }) => {
    ...
    const toggleBtn = page
      .locator('button:has([class*="eye"]), [aria-label*="password"], [aria-label*="show"]')
      .first()
...
test.describe("🪟 Interacción con Modales", () => {
  test("debe abrir y cerrar modales correctamente", async ({ page }) => {
    ...
    const modalTriggers = page.locator(
      'button:has-text("Recuperar"), button:has-text("Registrar"), a:has-text("Crear cuenta")'
    )
    ...
    if (count > 0) {
      ...
      if (modalVisible) {
        ...
        const closeBtn = modal
          .locator('button:has-text("×"), button:has-text("Cerrar"), button[aria-label*="close"]')
          .first()
