...
async function navigateToPanel(page: Page, panelName: string) {
  ...
  const navItem = page
    .locator(
      `[data-panel="${panelName}"], button:has-text("${panelName}"), a:has-text("${panelName}")`
    )
    .first()
...
async function closeModal(page: Page) {
  const closeBtn = page
    .locator(
      '[role="dialog"] button:has-text("×"), [role="dialog"] button:has-text("Cerrar"), [role="dialog"] [aria-label="Close"]'
    )
    .first()
...
test.describe("🛒 Flujo Completo de Ventas", () => {
  ...
  test("debe abrir modal de nueva venta", async ({ page }) => {
    ...
    const nuevaVentaBtn = page
      .locator(
        'button:has-text("Nueva Venta"), button:has-text("Registrar Venta"), button:has-text("+ Venta"), [aria-label*="venta"]'
      )
      .first()
  ...
  test("debe crear una venta completa con distribución GYA", async ({ page }) => {
    ...
    const nuevaVentaBtn = page
      .locator(
        'button:has-text("Nueva Venta"), button:has-text("Registrar Venta"), button:has-text("+ Venta")'
      )
      .first()
    ...
    const clienteInput = modal
      .locator(
        'input[placeholder*="cliente"], input[name*="cliente"], [data-testid="cliente-search"]'
      )
      .first()
    ...
    const agregarProductoBtn = modal
      .locator(
        'button:has-text("Agregar"), button:has-text("+ Producto"), button:has-text("Añadir")'
      )
      .first()
    ...
    const confirmarBtn = modal
      .locator(
        'button:has-text("Confirmar"), button:has-text("Crear Venta"), button:has-text("Guardar")'
      )
      .first()
...
test.describe("👥 Flujo de Clientes", () => {
  ...
  test("debe crear un nuevo cliente", async ({ page }) => {
    ...
    const nuevoClienteBtn = page
      .locator(
        'button:has-text("Nuevo Cliente"), button:has-text("+ Cliente"), button:has-text("Agregar Cliente")'
      )
      .first()
    ...
    const nombreInput = modal
      .locator('input[name*="nombre"], input[placeholder*="nombre"], input:first-of-type')
      .first()
    if (await nombreInput.isVisible()) {
      await nombreInput.fill(TEST_DATA.cliente.nombre)
    }

    const telefonoInput = modal
      .locator('input[name*="telefono"], input[type="tel"], input[placeholder*="teléfono"]')
      .first()
  ...
  test("debe registrar un abono a cliente", async ({ page }) => {
    ...
    const abonoBtn = page
      .locator(
        'button:has-text("Abono"), button:has-text("Cobrar"), button:has-text("Pago Cliente")'
      )
      .first()
    ...
    const clienteConDeuda = modal
      .locator('[class*="cliente"]:has-text("$"), button:has-text("Cliente")')
      .first()
    ...
    const bancoDestino = modal
      .locator('button:has-text("Bóveda"), button:has-text("Profit"), [data-banco]')
      .first()
...
test.describe("📦 Flujo de Órdenes de Compra", () => {
  ...
  test("debe crear una orden de compra", async ({ page }) => {
    const nuevaOCBtn = page
      .locator('button:has-text("Nueva OC"), button:has-text("Orden"), button:has-text("+ Compra")')
      .first()
    ...
    const distribuidorSelect = modal
      .locator('[class*="distribuidor"], select, [role="combobox"]')
      .first()
...
test.describe("🔄 Flujo de Transferencias entre Bancos", () => {
  ...
  test("debe realizar una transferencia entre bancos", async ({ page }) => {
    const transferenciaBtn = page
      .locator(
        'button:has-text("Transferencia"), button:has-text("Transferir"), button:has-text("Mover")'
      )
      .first()
    ...
    const bancoOrigen = modal
      .locator('[class*="banco"]:has-text("Monte"), button:has-text("Bóveda Monte")')
      .first()
    ...
    // Seleccionar banco destino
    const bancoDestino = modal
      .locator('[class*="banco"]:has-text("Profit"), button:has-text("Profit")')
      .first()
    if (await bancoDestino.isVisible({ timeout: 2000 }).catch(() => false)) {
      await bancoDestino.click()
    }
...
test.describe("💸 Flujo de Gastos", () => {
  ...
  test("debe registrar un gasto", async ({ page }) => {
    const gastoBtn = page
      .locator('button:has-text("Gasto"), button:has-text("Egreso"), button:has-text("+ Gasto")')
      .first()
    ...
    // Seleccionar banco origen
    const bancoOrigen = modal
      .locator('[class*="banco"], button:has-text("Profit"), button:has-text("Azteca")')
      .first()
    if (await bancoOrigen.isVisible({ timeout: 2000 }).catch(() => false)) {
      await bancoOrigen.click()
    }
...
test.describe("💰 Flujo de Ingresos", () => {
  ...
  test("debe registrar un ingreso", async ({ page }) => {
    const ingresoBtn = page
      .locator(
        'button:has-text("Ingreso"), button:has-text("Depósito"), button:has-text("+ Ingreso")'
      )
      .first()
...
test.describe("🚚 Flujo de Distribuidores", () => {
  ...
  test("debe crear un distribuidor", async ({ page }) => {
    const nuevoDistBtn = page
      .locator('button:has-text("Nuevo Distribuidor"), button:has-text("+ Distribuidor")')
      .first()
  ...
  test("debe registrar pago a distribuidor", async ({ page }) => {
    const pagoBtn = page
      .locator('button:has-text("Pagar"), button:has-text("Pago Distribuidor")')
      .first()
...
test.describe("📦 Flujo de Almacén", () => {
  ...
  test("debe mostrar inventario del almacén", async ({ page }) => {
    ...
    const visible = await inventario
      .first()
...
test.describe("🔥 Flujo E2E Completo", () => {
  test("debe ejecutar un ciclo completo de operaciones", async ({ page }) => {
    ...
    const actionButtons = page.locator(
      'button:has-text("Nueva"), button:has-text("Agregar"), button:has-text("+")'
    )
