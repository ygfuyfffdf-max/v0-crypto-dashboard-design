...
test.describe("🎯 Complete System Verification", () => {
  test.beforeEach(async ({ page }) => {
    // Go directly to dashboard for testing (skip 14s splash animation)
    await page.goto("/dashboard")
  })

  test.describe("1. Splash Screen & Initial Load", () => {
    test("should display ΚΟΣΜΟΣ warp drive experience", async ({ page }) => {
      // Wait for canvas (WebGL shader)
      const canvas = page.locator("canvas").first()
      await expect(canvas).toBeVisible({ timeout: 10000 })

      // Verify warp animation is running
      await page.waitForTimeout(2000)

      // After 14 seconds, should redirect to login
      // We skip the full animation in tests
    })

    test("should have WebGL canvas for warp animation", async ({ page }) => {
  ...
  test.describe("2. Dashboard 3D Components", () => {
    test("should load dashboard with Glass Gen-5 components", async ({ page }) => {
    ...
    test("should have animated counters and stats", async ({ page }) => {
      // Wait for AnimatedCounter components to render
