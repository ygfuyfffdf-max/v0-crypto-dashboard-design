      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-purple-200">
            Configuración granular de permisos y seguridad cuántica
          </p>
        </div>
                ...
                <div key={stepItem.id} className="flex items-center">
                  ...
                  {index < steps.length - 1 && (
                    <div className={`
                      ...
                      ${step > stepItem.id ? 'bg-purple-600' : 'bg-slate-600'}
                    `} />
                  )}
                </div>
              );
...
const BasicInfoStep: React.FC<StepProps> = ({ userData, onUpdate, onNext }) => {
  ...
  return (
    <Card className="p-8 bg-slate-800 border-purple-500">
...
const RoleSelectionStep: React.FC<StepProps> = ({ userData, onUpdate, onNext, onPrevious }) => {
  ...
  const availableRoles = [
    { value: 'bank_profit_manager', label: 'Bank Profit Manager', description: 'Gestión bancaria y de ganancias' },
    { value: 'user_admin', label: 'User Administrator', description: 'Administración de usuarios y permisos' },
    { value: 'security_monitor', label: 'Security Monitor', description: 'Monitoreo de seguridad y auditoría' },
    { value: 'financial_manager', label: 'Financial Manager', description: 'Gestión financiera general' },
  ...
  return (
    <Card className="p-8 bg-slate-800 border-purple-500">
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-white mb-2">Rol y Responsabilidades</h2>
            ...
            <div className="space-y-2">
              {availableRoles.map((role) => (
                <label key={role.value} className="flex items-center space-x-3">
                  ...
                  <span className="text-purple-300 text-sm">({role.description})</span>
                </label>
              ))}
            </div>
          </div>
        ...
        <div className="flex justify-between">
          <Button onClick={onPrevious} variant="outline" className="border-purple-500 text-purple-200">
...
const GranularPermissionsStep: React.FC<StepProps> = ({ userData, onUpdate, onNext, onPrevious }) => {
  ...
  return (
    <Card className="p-8 bg-slate-800 border-purple-500">
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-white mb-2">Permisos Granulares por Panel</h2>
          <p className="text-purple-200">
            Seleccione los paneles a los que este usuario tendrá acceso y configure los permisos específicos.
          </p>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-start space-x-3">
                  <div className={`p-2 rounded-lg ${panel.bgColor}`}>
                    <Icon className={`w-6 h-6 ${panel.color}`} />
                  </div>
                  <div className="flex-1">
                    <h3 className="font-semibold text-white">{panel.label}</h3>
                    <p className="text-sm text-purple-200 mb-2">{panel.description}</p>
                    <Badge className={`
                      ${panel.risk === 'CRITICAL' ? 'bg-red-600' :
                        panel.risk === 'HIGH' ? 'bg-orange-600' :
                        panel.risk === 'MEDIUM' ? 'bg-yellow-600' : 'bg-green-600'}
                    `}>
                      {panel.risk} RISK
                    </Badge>
                  </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-600">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <select
                        ...
                        className="bg-slate-700 border-purple-500 text-white text-sm rounded px-2 py-1"
        ...
        <div className="bg-slate-700/50 rounded-lg p-4">
          <h3 className="font-semibold text-white mb-3">Resumen de Permisos Seleccionados</h3>
          <div className="space-y-2">
            {selectedPanels.length === 0 ? (
              ...
              selectedPanels.map(panelId => {
                ...
                return (
                  <div key={panelId} className="flex items-center justify-between bg-slate-800 rounded p-2">
                    <div className="flex items-center space-x-2">
                      {panel && <panel.icon className={`w-4 h-4 ${panel.color}`} />}
...
const RestrictionsStep: React.FC<StepProps> = ({ userData, onUpdate, onNext, onPrevious }) => {
  ...
  return (
    <Card className="p-8 bg-slate-800 border-purple-500">
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-white mb-2">Restricciones y Condiciones</h2>
          <p className="text-purple-200">
        ...
        <div className="space-y-4">
          ...
          {restrictions.timeWindows.map((window, index) => (
            <Card key={index} className="p-4 bg-slate-700 border-purple-500">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                  <label className="block text-sm font-medium text-purple-200 mb-1">Hora Inicio</label>
                  <Input
                    type="time"
                    value={window.start}
                    onChange={(e) => updateTimeWindow(index, 'start', e.target.value)}
                    className="bg-slate-600 border-purple-500 text-white"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-purple-200 mb-1">Hora Fin</label>
                  <Input
                    type="time"
                    value={window.end}
                ...
                <div>
                  <label className="block text-sm font-medium text-purple-200 mb-1">Zona Horaria</label>
                  <Select
                    value={window.timezone}
                    onValueChange={(value) => updateTimeWindow(index, 'timezone', value)}
                  >
                    <SelectTrigger className="bg-slate-600 border-purple-500">
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="mt-4">
                <label className="block text-sm font-medium text-purple-200 mb-2">Días Permitidos</label>
...
const BiometricStep: React.FC<StepProps> = ({ userData, onUpdate, onNext, onPrevious }) => {
  ...
  return (
    <Card className="p-8 bg-slate-800 border-purple-500">
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-white mb-2">Biometría y Seguridad</h2>
          <p className="text-purple-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card className="p-6 bg-slate-700 border-purple-500">
            ...
            <div className="space-y-4">
              ...
              <label className="flex items-center justify-between">
                ...
                <input
                  ...
                  onChange={(e) => setBiometric({ ...biometric, faceRecognition: e.target.checked })}
                  className="rounded border-purple-500 bg-slate-600"
                />
              </label>
              <label className="flex items-center justify-between">
        ...
        <div className="bg-slate-700/50 rounded-lg p-4">
          <h3 className="font-semibold text-white mb-3">Resumen de Seguridad</h3>
          <div className="space-y-2">
...
const ApprovalChainStep: React.FC<StepProps> = ({ userData, onUpdate, onNext, onPrevious }) => {
  ...
  const approvalTypes = [
    ...
    { key: 'complianceApproval', label: 'Aprobación de Cumplimiento', icon: FileText, required: false },
  ...
  return (
    <Card className="p-8 bg-slate-800 border-purple-500">
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-white mb-2">Cadena de Aprobaciones</h2>
          <p className="text-purple-200">
        ...
        <div className="space-y-4">
          {approvalTypes.map((approval) => {
            ...
            return (
              <Card key={approval.key} className="p-4 bg-slate-700 border-purple-500">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      ...
                      <div className={`
                        ...
                        ${approval.required ? 'opacity-50 cursor-not-allowed' : ''}
                      `} />
                    </label>
                  </div>
                </div>
        ...
        <div className="bg-slate-700/50 rounded-lg p-4">
          <h3 className="font-semibold text-white mb-3">Flujo de Aprobación</h3>
          <div className="space-y-2">
...
const ReviewStep: React.FC<StepProps & { onSubmit: () => void; isSubmitting: boolean }> = ({
  ...
  return (
    <Card className="p-8 bg-slate-800 border-purple-500">
      <div className="space-y-6">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-white mb-2">Revisión y Confirmación</h2>
          <p className="text-purple-200">
            Revise toda la configuración antes de crear el usuario
          </p>
        </div>
...
="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-white font-medium">{userData.basicInfo.firstName} {userData.basicInfo.lastName}</p>
              </div>
              <div>
                <span className="text-purple-200">Email:</span>
                <p className="text-white font-medium">{userData.basicInfo.email}</p>
              </div>
              <div>
                <span className="text-purple-200">Departamento:</span>
                <p className="text-white font-medium">{userData.basicInfo.department}</p>
              </div>
              <div>
                <span className="text-purple-200">Posición:</span>
                <p className="text-white font-medium">{userData.basicInfo.position}</p>
              </div>
            </div>
          </TabsContent>

          <TabsContent value="role" className="space-y-4">
            <h3 className="text-lg font-semibold text-white mb-4">Rol y Permisos</h3>
            <div className="space-y-4">
              <div>
                <span className="text-purple-200">Rol Principal:</span>
                <p className="text-white font-medium">{userData.role.primaryRole}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <Badge className="bg-orange-600 ml-2">{userData.role.clearanceLevel}</Badge>
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="security" className="space-y-4">
            ...
            <div className="mt-4">
              ...
              <Badge className="bg-yellow-600 ml-2">{(userData.restrictions.riskThreshold * 100).toFixed(0)}%</Badge>
            </div>
          </TabsContent>

          <TabsContent value="approvals" className="space-y-4">
            <h3 className="text-lg font-semibold text-white mb-4">Cadena de Aprobaciones</h3>
            <div className="space-y-2">
