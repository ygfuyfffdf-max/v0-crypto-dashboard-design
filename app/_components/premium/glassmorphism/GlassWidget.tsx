...
import { ChevronDown, Brain, Sparkles, Zap, Eye, Mic, Settings, X } from 'lucide-react'
...
export const GlassWidget: React.FC<GlassWidgetProps> = ({
  ...
  const optimizeGlassEffect = useCallback((options: {
    blur: number
    opacity: number
    border: string
  ...
  const generatePredictions = useCallback(async (context: AIContext): Promise<string[]> => {
...
const QuickActions: React.FC<{
  ...
  const actions = [
    ...
    { id: 'settings', label: 'Config', icon: Settings, active: false, onClick: () => onAction('open-settings') }
  ].filter(action => 
    action.id !== 'voice' || capabilities.voiceControl
  ).filter(action =>
    action.id !== 'vr' || capabilities.vrArSupport
  )
