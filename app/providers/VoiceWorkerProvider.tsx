...
export function VoiceWorkerProvider({ children }: { children: React.ReactNode }) {
  ...
  const pendingPromises = useRef<
    Map<string, { resolve: (value: unknown) => void; reject: (error: unknown) => void }>
  >(new Map())
  ...
  useEffect(() => {
    ...
    try {
      ...
      logger.warn('[VoiceWorker] No se pudo inicializar worker, funcionando sin él', {
        context: 'VoiceWorkerProvider',
      })
    }
  ...
  const sendAIQuery = useCallback((text: string): Promise<{ intent: string; success: boolean }> => {
    return new Promise((resolve, reject) => {
      ...
      pendingPromises.current.set('ai_query', {
  ...
  return <VoiceWorkerContext.Provider value={value}>{children}</VoiceWorkerContext.Provider>
}
