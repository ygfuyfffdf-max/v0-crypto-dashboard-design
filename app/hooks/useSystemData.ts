...
export function useSystemData(): SystemDataResult {
  ...
  const {
    ...
    error: errorVentas,
    refetch: refreshVentas,
  } = useVentasData()
  const {
    data: clientesRaw,
    loading: loadingClientes,
    error: errorClientes,
    refetch: refreshClientes,
  } = useClientesData()
  const {
    data: distribuidoresRaw,
    loading: loadingDist,
    error: errorDist,
    refetch: refreshDist,
  } = useDistribuidoresData()
  const {
    data: ordenesCompraRaw,
    loading: loadingOC,
    error: errorOC,
    refetch: refreshOC,
  } = useOrdenesCompraData()

  // Extraer productos de almacenData
  const productosRaw = almacenData.productos

  // Castear datos con tipos seguros
  ...
  const clientes = clientesRaw.map((c) => ({
    ...
    email: c.email,
    telefono: c.telefono,
  })) as SystemDataResult['clientes']

  const distribuidores = distribuidoresRaw.map((d) => ({
    id: d.id,
    nombre: d.nombre,
    deudaTotal: d.saldoPendiente,
    totalOrdenesCompra: d.totalOrdenesCompra,
    empresa: d.empresa,
  })) as SystemDataResult['distribuidores']

  const ordenesCompra = ordenesCompraRaw.map((oc) => ({
    id: oc.id,
    estado: oc.estado,
    costoTotal: oc.total,
    deuda: oc.montoRestante,
    distribuidorId: oc.distribuidorId,
  })) as SystemDataResult['ordenesCompra']
  ...
  const errors = [errorProd, errorVentas, errorClientes, errorDist, errorOC]
  ...
  const kpis = useMemo((): KPIs => {
    ...
    const totalRevenue = ventas.reduce((acc, v) => {
      const total =
        typeof v.total === 'number'
          ? v.total
          : typeof v.precioTotalVenta === 'number'
            ? v.precioTotalVenta
            : 0
      return acc + total
    }, 0)
    ...
    // Stock total y valor
    const stockData = productos.reduce(
      (acc, p) => {
        const stock =
    ...
    // Órdenes pendientes
    const pendingOrders = ordenesCompra.filter(
      (oc) => oc.estado === 'pendiente' || oc.estado === 'parcial',
    ).length
...
export function useDashboardStats() {
  ...
  const lowStockProducts = productos.filter((p) => {
    const stock =
      typeof p.stockActual === 'number' ? p.stockActual : typeof p.stock === 'number' ? p.stock : 0
...
/**
 * Hook para el panel de Bancos (legacy - usar useBancosData de useDataHooks)
 * @deprecated Usar useBancosData y useMovimientosData de useDataHooks
 */
export function useBancoPanel(bancoId: string) {
  const { data: bancos, loading: bancosLoading } = useBancosData()
...
export function useVentasStats() {
  ...
  const stats = useMemo(() => {
    const completadas = ventas.filter(
      (v) => v.estadoPago === 'completo' || v.estado === 'completada',
    )
    const pendientes = ventas.filter(
      (v) => v.estadoPago === 'pendiente' || v.estadoPago === 'parcial',
    )
