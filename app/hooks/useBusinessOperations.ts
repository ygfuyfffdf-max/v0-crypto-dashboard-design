...
export function useCalculoVentaPreview() {
  ...
  const calcular = useCallback(
    (cantidad: number, precioVenta: number, precioCompra: number, precioFlete?: number) => {
...
export function useValidarStock() {
  ...
  const validar = useCallback(
    (stockActual: number, cantidadSolicitada: number): ValidacionStock => {
...
export function useSeleccionBanco(bancosDisponibles: BancoOption[]) {
  ...
  const validarSaldo = useCallback(
    (bancoId: BancoId, montoRequerido: number): boolean => {
  ...
  const getBanco = useCallback(
    (bancoId: BancoId): BancoOption | undefined => {
