 ...
 */
function calculateArbitrageOpportunity(feed: MarketLiveFeed): ArbitrageOpportunity | null {
  ...
  if (cryptoVsPhysical > 0) {
    ...
    profitPercent =
      ((sources.street_average_sell - sources.banxico_fix) / sources.banxico_fix) * 100
  }
  ...
  useEffect(() => {
    const loadHistorical = async () => {
      ...
      try {
        ...
        const q = query(historyRef, orderBy('timestamp', 'desc'), limit(HISTORICAL_LIMIT))
