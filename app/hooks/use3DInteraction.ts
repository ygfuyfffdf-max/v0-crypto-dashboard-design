 ...
 */
export function useHoverScale(baseScale = 1, hoverScale = 1.2, lerpFactor = 0.1) {
  const [isHovered, setIsHovered] = useState(false)
  const scaleRef = useRef(baseScale)
  const targetScale = isHovered ? hoverScale : baseScale

  useFrame(() => {
    scaleRef.current = THREE.MathUtils.lerp(scaleRef.current, targetScale, lerpFactor)
  })
 ...
 */
export function useMouseLookAt(intensity = 0.3, smoothing = 0.1) {
  ...
  useFrame(() => {
    ...
    rotationRef.current.x = THREE.MathUtils.lerp(rotationRef.current.x, targetX, smoothing)
    rotationRef.current.y = THREE.MathUtils.lerp(rotationRef.current.y, targetY, smoothing)
  })
 ...
 */
export function useFloatingAnimation(amplitude = 0.1, frequency = 1, phase = 0) {
 ...
 */
export function usePressAnimation(normalScale = 1, pressedScale = 0.95, lerpFactor = 0.2) {
...
export function useParticleRepulsion(
  ...
  useFrame(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      const i3 = i * 3
      const x = positionsRef.current[i3] ?? 0
      const y = positionsRef.current[i3 + 1] ?? 0
      ...
      if (distance < repulsionRadius && distance > 0) {
        ...
        if (positionsRef.current[i3] !== undefined) {
          positionsRef.current[i3] = (positionsRef.current[i3] ?? 0) + nx * force
        }
        if (positionsRef.current[i3 + 1] !== undefined) {
          positionsRef.current[i3 + 1] = (positionsRef.current[i3 + 1] ?? 0) + ny * force
        }
      }

      // Volver a la posición original con elasticidad
      const posX = positionsRef.current[i3] ?? 0
      const posY = positionsRef.current[i3 + 1] ?? 0
      const posZ = positionsRef.current[i3 + 2] ?? 0
      const targetX = targetPositionsRef.current[i3] ?? 0
      const targetY = targetPositionsRef.current[i3 + 1] ?? 0
      const targetZ = targetPositionsRef.current[i3 + 2] ?? 0

      if (positionsRef.current[i3] !== undefined) {
        positionsRef.current[i3] = (positionsRef.current[i3] ?? 0) + (targetX - posX) * elasticity
      }
      if (positionsRef.current[i3 + 1] !== undefined) {
        positionsRef.current[i3 + 1] =
          (positionsRef.current[i3 + 1] ?? 0) + (targetY - posY) * elasticity
      }
      if (positionsRef.current[i3 + 2] !== undefined) {
        positionsRef.current[i3 + 2] =
          (positionsRef.current[i3 + 2] ?? 0) + (targetZ - posZ) * elasticity
      }
    }
  })
 ...
 */
export function usePulseAnimation(minScale = 0.95, maxScale = 1.05, frequency = 2) {
