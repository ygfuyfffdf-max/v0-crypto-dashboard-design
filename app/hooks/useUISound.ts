 ...
 */
export function useUISound(soundType: SoundType = 'click', options: SoundOptions = {}) {
  const { volume = 0.5, playbackRate = 1, interrupt = true, soundEnabled = true } = options
  ...
  useEffect(() => {
    ...
    audio.addEventListener('error', () => {
      ...
      logger.debug(`Sonido no encontrado: ${SOUND_URLS[soundType]}`, { context: 'useUISound' })
    })
