...
export function useTrazabilidadCliente(clienteId: string | null): TrazabilidadCliente {
  ...
  useEffect(() => {
    ...
    const unsubAbonos = onSnapshot(
      ...
      (err) => logger.warn('Abonos query error', { context: 'useTrazabilidad', data: err }),
    )

    // Cargar datos del cliente
    const clienteQuery = query(collection(db, 'clientes'), where('id', '==', clienteId))
  ...
  const metricas = useMemo(() => {
    ...
    const totalPagado =
      ventas.reduce((sum, v) => sum + (v.montoPagado ?? 0), 0) +
    ...
    if (ventas.length > 1) {
      ...
      if (fechas.length > 1) {
        const diasTotales =
          (fechas[fechas.length - 1].getTime() - fechas[0].getTime()) / (1000 * 60 * 60 * 24)
 ...
 */
export function useTrazabilidadDistribuidor(
  distribuidorId: string | null,
): TrazabilidadDistribuidor {
  ...
  useEffect(() => {
    ...
    const unsubPagos = onSnapshot(
      ...
      (err) => logger.warn('Pagos query error', { context: 'useTrazabilidad', data: err }),
    )

    // Cargar distribuidor
    const distQuery = query(collection(db, 'distribuidores'), where('id', '==', distribuidorId))
...
export function useTrazabilidadOC(ocId: string | null): TrazabilidadOC {
  ...
  useEffect(() => {
    ...
    // Suscripción a la OC
    const ocQuery = query(collection(db, 'ordenes_compra'), where('id', '==', ocId))
  ...
  const metricas = useMemo(() => {
    ...
    const ingresoGenerado = ventas.reduce(
      (sum, v) => sum + (v.precioTotalVenta ?? v.ingreso ?? 0),
      0,
    )
...
export function useAnalisisGastosAbonos(filtros?: FiltrosAnalisis): AnalisisGastosAbonos {
  ...
  const resumen = useMemo(() => {
    ...
    // Gastos por banco
    const gastosPorBanco = gastos.reduce(
      (acc, g) => {
