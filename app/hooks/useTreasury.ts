...
export function useTreasury(currentExchangeRate: number = 18.5): UseTreasuryReturn {
  ...
  // Actualizar posición
  const updatePosition = useCallback(async (updates: Partial<TreasuryPosition>): Promise<void> => {
    try {
  ...
  const getOptimalPrice = useCallback(
    (competitorPrice: number, mode: 'buy' | 'sell') => {
      if (!state.position) {
        return { price: competitorPrice, adjustment: 0, reason: 'Sin datos de inventario' }
      }

      const inventoryPercent =
        state.position.physical_vault_usd / DEFAULT_VAULT_CAPACITIES.physical_vault_usd
  ...
  const getUtilizationPercent = useCallback(
    (field: keyof typeof DEFAULT_VAULT_CAPACITIES): number => {
  ...
  const isLowInventory = useCallback(
    (field: keyof typeof DEFAULT_VAULT_CAPACITIES): boolean => {
