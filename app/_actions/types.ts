...
export const CreateVentaSchema = z.object({
  ...
  ocRelacionada: z.string().min(1, 'Orden de compra requerida'),
...
export const UpdateMovimientoSchema = z.object({
  id: z.string().min(1, 'ID requerido'),
  concepto: z.string().min(1, 'Concepto requerido').optional(),
  referencia: z.string().optional(),
  observaciones: z.string().optional(),
})
...
export const CreateProductoSchema = z.object({
  ...
  minimo: z.number().int().min(0).optional().default(0),
...
// ═══════════════════════════════════════════════════════════════
// VENTA COMPLETA - Schema con todos los campos CHRONOS 2026
// ═══════════════════════════════════════════════════════════════

export const CreateVentaCompletaSchema = z
  .object({
    // Cliente - puede ser ID existente o nombre nuevo
    clienteId: z.string().optional(),
    ...
    // Trazabilidad de lotes (opcional - si no se especifica, FIFO automático)
    origenLotes: z
      .array(
        z.object({
          ocId: z.string(),
  ...
  .refine((data) => data.clienteId || data.clienteNombre, {
    message: 'Debe especificar clienteId o clienteNombre',
    path: ['clienteId'],
  })
...
// ═══════════════════════════════════════════════════════════════
// ORDEN COMPRA COMPLETA - Schema con entrada a almacén
// ═══════════════════════════════════════════════════════════════

export const CreateOrdenCompletaSchema = z
  .object({
    // Distribuidor - puede ser ID existente o nombre nuevo
    distribuidorId: z.string().optional(),
    distribuidorNombre: z.string().optional(),
    // Producto
    productoId: z.string().optional(), // Added for linking to inventory
    cantidad: z.number().int().positive('Cantidad debe ser positiva'),
    precioUnitario: z.number().positive('Precio debe ser positivo'),
    fleteUnitario: z.number().min(0).default(0),
    iva: z.number().min(0).default(0),
    // Banco de donde sale el pago (si se paga)
  ...
  .refine((data) => data.distribuidorId || data.distribuidorNombre, {
    message: 'Debe especificar distribuidorId o distribuidorNombre',
    path: ['distribuidorId'],
  })

export type CreateOrdenCompletaInput = z.infer<typeof CreateOrdenCompletaSchema>

// ═══════════════════════════════════════════════════════════════
// ABONO A VENTA - Schema completo
// ═══════════════════════════════════════════════════════════════

export const AbonoVentaCompletaSchema = z.object({
  ventaId: z.string().min(1, 'Venta requerida'),
  monto: z.number().positive('Monto debe ser positivo'),
  concepto: z.string().optional(),
})

export type AbonoVentaCompletaInput = z.infer<typeof AbonoVentaCompletaSchema>

// ═══════════════════════════════════════════════════════════════
// TRANSFERENCIA ENTRE BANCOS
// ═══════════════════════════════════════════════════════════════

export const TransferenciaBancosSchema = z
  .object({
    bancoOrigenId: z.string().min(1, 'Banco origen requerido'),
    bancoDestinoId: z.string().min(1, 'Banco destino requerido'),
    monto: z.number().positive('Monto debe ser positivo'),
  ...
  .refine((data) => data.bancoOrigenId !== data.bancoDestinoId, {
    message: 'Origen y destino no pueden ser el mismo banco',
    path: ['bancoDestinoId'],
  })
