...
interface ChronosState {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // ACCIONES — PRODUCTOS/ALMACÉN
  // ═══════════════════════════════════════════════════════════════════════
  crearProducto: (producto: Omit<Producto, 'id' | 'createdAt' | 'updatedAt'>) => string
  actualizarProducto: (id: string, cambios: Partial<Producto>) => void
  eliminarProducto: (id: string) => void
  ajustarStock: (id: string, cantidad: number, tipo: 'entrada' | 'salida' | 'ajuste') => void
  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        ventas: [],
        ordenesCompra: [],
        // Clientes iniciales para que los modales funcionen
        clientes: [
          {
            id: 'bodega_mp',
            nombre: 'Bodega M-P',
          ...
          },
          {
            ...
            deudaTotal: 0,
            numeroCompras: 0,
            keywords: ['wero', 'benavides'],
            estado: 'activo' as const,
            createdAt: new Date().toISOString(),
          },
        ],
        // Distribuidores iniciales para que los modales funcionen
        distribuidores: [
          {
            id: 'q_maya',
            nombre: 'Q-MAYA',
        ...
        crearVenta: (ventaData: CrearVentaInput) => {
          ...
          const oc = ventaData.ocRelacionada
            ? state.ordenesCompra.find((o) => o.id === ventaData.ocRelacionada)
            : null
          const precioCompra = ventaData.precioCompra || oc?.costoPorUnidad || 0
          const precioFlete =
            ventaData.flete === 'Aplica' ? (ventaData.fleteUtilidad || 0) / ventaData.cantidad : 0
          ...
          // ═══════════════════════════════════════════════════════════════════
          // LÓGICA SAGRADA: precioTotalVenta = (precioVenta + flete) × cantidad
          // El cliente paga precioVenta + flete por unidad
          // Ejemplo: (10,000 + 500) × 10 = $105,000
          // ═══════════════════════════════════════════════════════════════════
          const totalVenta = distribucion.total // Ya calculado correctamente en calcularDistribucionVenta
          ...
          set((state) => {
            ...
            if (distribucionCapital.bovedaMonte > 0) {
              nuevosBancos.boveda_monte = {
                ...nuevosBancos.boveda_monte,
                capitalActual:
                  nuevosBancos.boveda_monte.capitalActual + distribucionCapital.bovedaMonte,
                historicoIngresos:
                  nuevosBancos.boveda_monte.historicoIngresos + distribucion.bovedaMonte,
                updatedAt: now,
              }
            }
            ...
            if (distribucionCapital.utilidades > 0) {
              nuevosBancos.utilidades = {
                ...nuevosBancos.utilidades,
                capitalActual:
                  nuevosBancos.utilidades.capitalActual + distribucionCapital.utilidades,
                historicoIngresos:
                  nuevosBancos.utilidades.historicoIngresos + distribucion.utilidades,
                updatedAt: now,
              }
            }
            ...
            const nuevosClientes = state.clientes.map((c) => {
              if (c.id === ventaData.clienteId) {
                const ventasCliente = [
                  ...state.ventas.filter((v) => v.clienteId === c.id),
                  nuevaVenta,
                ]
            ...
            // Calcular totales
            const totalCapital = Object.values(nuevosBancos).reduce(
              (acc, b) => acc + b.capitalActual,
              0,
            )
            const totalDeudaClientes = nuevosClientes.reduce(
              (acc, c) => acc + (c.deudaTotal || 0),
              0,
            )
            ...
            const nuevasVentas = state.ventas.map((v) => {
              if (v.id === ventaId) {
                return {
                  ...
                  estatus:
                    resultado.nuevoEstado === 'completo'
                      ? ('Pagado' as const)
                      : resultado.nuevoEstado === 'parcial'
                        ? ('Parcial' as const)
                        : ('Pendiente' as const),
                  updatedAt: now,
                }
          ...
          set((state) => {
            ...
            const nuevosDistribuidores = state.distribuidores.map((d) => {
              if (d.id === ocData.distribuidorId) {
                const ordenesDistribuidor = [
                  ...state.ordenesCompra.filter((o) => o.distribuidorId === d.id),
                  nuevaOC,
                ]
            ...
            const totalDeudaDistribuidores = nuevosDistribuidores.reduce(
              (acc, d) => acc + (d.deudaTotal || 0),
              0,
            )
            const stockTotal = [...state.ordenesCompra, nuevaOC].reduce(
              (acc, oc) => acc + oc.stockActual,
              0,
            )
          ...
          set((state) => {
            ...
            const nuevosMovimientos = [
              ...state.movimientos,
              {
                id: generarIdMovimiento(),
                bancoId: bancoOrigen,
                tipoMovimiento: 'pago' as TipoMovimiento,
                fecha: now,
                monto: -monto,
        ...
        crearProducto: (producto) => {
          const now = new Date().toISOString()
          const id = `PROD${String(get().productos.length + 1).padStart(4, '0')}`
          ...
          set((state) => ({
            productos: [
              ...state.productos,
              {
                ...
                totalEntradas: producto.totalEntradas || 0,
                totalSalidas: producto.totalSalidas || 0,
        ...
        actualizarProducto: (id, cambios) => {
          ...
          set((state) => ({
            productos: state.productos.map((p) =>
              p.id === id ? { ...p, ...cambios, updatedAt: now } : p,
            ),
            lastSync: Date.now(),
          }))
        },
        ...
        ajustarStock: (id, cantidad, tipo) => {
          ...
          set((state) => ({
            productos: state.productos.map((p) => {
              ...
              let nuevoStock = p.stockActual
              let nuevasEntradas = p.totalEntradas || 0
              let nuevasSalidas = p.totalSalidas || 0

              switch (tipo) {
                case 'entrada':
                  nuevoStock += cantidad
                  nuevasEntradas += cantidad
                  break
                case 'salida':
                  nuevoStock = Math.max(0, nuevoStock - cantidad)
                  nuevasSalidas += cantidad
                  break
                case 'ajuste':
        ...
        recalcularTodo: () => {
          ...
          const nuevosDistribuidores = state.distribuidores.map((d) => {
            const ordenesDistribuidor = state.ordenesCompra.filter(
              (oc) => oc.distribuidorId === d.id,
            )
            const updates = recalcularDistribuidor(d, ordenesDistribuidor)
            return { ...d, ...updates }
          })
...
export const selectVentasPendientes = (state: ChronosState) =>
  state.ventas.filter((v) => v.estadoPago !== 'completo')
export const selectVentasHoy = (state: ChronosState) => {
  const hoy = new Date().toISOString().split('T')[0]
  if (!hoy) return []
  return state.ventas.filter((v) => {
    const fecha = v.fecha as string
    return fecha?.startsWith(hoy) || false
  })
}
