...
import type { BancoId, Cliente, Distribuidor, Movimiento, OrdenCompra, Venta } from '@/app/types'
import { create } from 'zustand'
import { devtools, persist } from 'zustand/middleware'
import { calcularVentaCompleta, FLETE_DEFAULT } from '../formulas'
    ...
    persist(
      (set, get) => ({
        ...
        getKPIs: () => {
          ...
          const ventasHoy = state.ventas
            .filter((v) => {
              const fechaVenta = typeof v.fecha === 'string' ? v.fecha : new Date().toISOString()
              return (hoy && fechaVenta?.startsWith(hoy)) || false
            })
        ...
        addVenta: (ventaData) => {
          ...
          const precioFlete =
            ventaData.flete === 'Aplica' ? ventaData.fleteUtilidad || FLETE_DEFAULT : 0
            ...
            const nuevasVentas = state.ventas.map((v) =>
              v.id === ventaId
                ? {
                    ...
                    estatus: (nuevoEstado === 'completo' ? 'Pagado' : 'Parcial') as
                      | 'Pagado'
                      | 'Parcial'
                      | 'Pendiente',
                    updatedAt: now,
        ...
        addCliente: (clienteData) => {
          ...
          logger.info(`Cliente creado: ${id}`, {
            context: 'DataStore',
            data: { id, nombre: clienteData.nombre },
          })
          return id
        },
        ...
        addDistribuidor: (distribuidorData) => {
          ...
          logger.info(`Distribuidor creado: ${id}`, {
            context: 'DataStore',
            data: { id, nombre: distribuidorData.nombre },
          })
          return id
        },
        ...
        addOrdenCompra: (ordenData) => {
          ...
          logger.info(`Orden de compra creada: ${id}`, {
            context: 'DataStore',
            data: { id, distribuidor: ordenData.distribuidor },
          })
          return id
        },
        ...
        pagarOrdenCompra: (ocId, monto, bancoId) => {
          ...
          logger.info('Pago a OC registrado', {
            context: 'DataStore',
            data: { ocId, monto, nuevoEstado },
          })
          return true
        },
        ...
        addMovimiento: (movData) => {
          ...
          set((state) => {
            const newBancos = state.bancos.map((banco) => {
              if (banco.id === bancoId) {
                if (tipo === 'ingreso' || tipo === 'transferencia_entrada' || tipo === 'abono') {
                  return {
                    ...banco,
          ...
          logger.info(`Movimiento creado: ${id}`, {
            context: 'DataStore',
            data: { id, bancoId, monto },
          })
          return id
        },
...
export function useLocalDistribuidores() {
  ...
  return {
    ...
    error: null,
    isConnected: true,
    add: async (item: Omit<Distribuidor, 'id' | 'createdAt' | 'updatedAt'>) =>
      addDistribuidor(item),
...
export function useLocalMovimientos(bancoId?: BancoId) {
  ...
  const filtered = bancoId ? movimientos.filter((m) => m.bancoId === bancoId) : movimientos
