...
/**
 * IDs de bancos válidos en el sistema (snake_case estandarizado)
 */
 ...
 */
export const CrearVentaSchema = VentaBaseSchema.refine(
...
/**
 * Schema para la API de creación de venta
 * - Valida solo los campos requeridos de entrada
 * - El resto se calcula en el servidor
 * - productoId es opcional (puede derivarse de la OC)
 * - ocRelacionada es requerido para trazabilidad
 */
export const CrearVentaAPISchema = z.object({
  clienteId: z.string().min(1, 'El ID del cliente es requerido'),
  productoId: z.string().optional(), // Opcional - puede venir de la OC
  cantidad: CantidadSchema,
  precioVentaUnidad: MontoSchema,
  precioCompraUnidad: z.number().min(0, 'El precio de compra no puede ser negativo').optional(),
  precioFlete: z.number().min(0, 'El flete no puede ser negativo').optional(),
  montoPagado: z.number().min(0, 'El monto pagado no puede ser negativo').optional(),
  observaciones: z.string().max(500).optional(),
  ocRelacionada: z.string().min(1, 'La orden de compra es requerida para trazabilidad'),
})
...
export type CrearVentaAPIInput = z.infer<typeof CrearVentaAPISchema>
 ...
 */
export function validarTransferencia(data: unknown): {
  success: boolean
  data?: Transferencia
  errors?: string[]
} {
 ...
 */
export function validarAbono(data: unknown): {
  success: boolean
  data?: AbonoCliente
  errors?: string[]
} {
