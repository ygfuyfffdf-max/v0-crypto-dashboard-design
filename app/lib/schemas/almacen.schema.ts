...
export const CategoriaProductoSchema = z
  .enum(['General', 'Electrónica', 'Ropa', 'Alimentos', 'Construcción', 'Automotriz', 'Otros'])
  .default('General')

/**
 * Tipos de movimiento de almacén
 */
export const TipoMovimientoAlmacenSchema = z.enum(['entrada', 'salida', 'ajuste'])
...
export const CrearProductoSchema = z
  .object({
    ...
    /** Unidad de medida */
    unidad: z.string().max(20, 'La unidad no puede exceder 20 caracteres').default('unidades'),
  ...
  .refine((data) => data.stockMaximo === undefined || data.stockMaximo >= data.stockMinimo, {
...
export const SalidaAlmacenSchema = z.object({
  ...
  destino: z.string().min(1, 'El destino es requerido').max(100),

  /** Referencia a venta (opcional) */
  ventaId: z.string().optional(),

  /** Motivo de la salida */
  motivo: z.string().max(200).default('Salida por venta'),
...
export const AjusteInventarioSchema = z.object({
  ...
  /** Motivo del ajuste */
  motivo: z.string().min(1, 'El motivo es requerido').max(200),
