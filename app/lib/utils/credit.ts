...
export function procesarPago(
  ...
  const pagoExcedente =
    pago.monto > distribuidor.deudaActual ? pago.monto - distribuidor.deudaActual : 0
...
export function calcularFacturasVencidas(
  ...
  pendientes.forEach((factura) => {
    const fechaFactura = toDateSafe(factura.fecha)
  ...
  facturasMasVencidas.sort((a, b) => {
    const fechaA = toDateSafe(a.fecha)
    const fechaB = toDateSafe(b.fecha)
    return fechaA.getTime() - fechaB.getTime()
  })

  const diasPromediomora =
    facturasMasVencidas.length > 0 ? diasMoraTotal / facturasMasVencidas.length : 0
 ...
 */
export function analizarCredito(distribuidor: Distribuidor, facturas: Factura[]): AnalisisCredito {
  const creditoDisponible = calcularCreditoDisponible(distribuidor)
  const porcentajeUtilizado =
    distribuidor.creditoTotal > 0 ? (distribuidor.deudaActual / distribuidor.creditoTotal) * 100 : 0
...
export function generarEstadoCuenta(
  ...
  facturas.forEach((factura) => {
    const fecha = toDateSafe(factura.fecha)
