...
export function calcularDistribucionVenta(data: VentaData): DistribucionVenta {
  ...
  // Distribución:
  // - bovedaMonte: costo del producto (va al banco del distribuidor)
  // - fletes: costo del flete (va al banco de fletes)
  // - utilidades: ganancia neta (precioVenta - costo - flete)
 ...
 */
export function necesitaReabastecimiento(stockActual: number, stockMinimo: number): boolean {
 ...
 */
export function calcularCapitalTotal(bancos: Array<{ saldo: number }>): number {
 ...
 */
export function proyectarValorFuturo(valores: number[], periodosFuturos: number): number {
 ...
 */
export function calcularDeudaTotalClientes(clientes: Array<{ deudaTotal: number }>): number {
