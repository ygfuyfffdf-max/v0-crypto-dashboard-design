 ...
 */
export function formatQuantity(value: number, unit: string, options?: { plural?: string }): string {
 ...
 * Formatear solo hora
 */
export function formatTime(date: Date | string | number, options?: { seconds?: boolean }): string {
...
export function formatStatus(status: string): {
  ...
  const statuses: Record<string, { label: string; color: string; bgColor: string }> = {
    ...
    enviado: { label: 'Enviado', color: 'text-violet-500', bgColor: 'bg-violet-500/10' },
