...
export function calcularCorte(
  ...
  const movimientosPeriodo = movimientos.filter((m) => {
    const fechaMov = toDateSafe(m.fecha)
    return m.banco === banco && fechaMov >= fechaInicio && fechaMov <= fechaFin
  })
...
export function cerrarCorte(
  ...
  if (discrepancia) {
    logger.warn(
      `Discrepancia en corte de caja: ${corte.banco}, diferencia: ${diferencia}, severidad: ${discrepancia.severidad}`,
    )
  }
...
export function detectarDiscrepancias(
  ...
  const porcentaje =
    corte.saldoEsperado > 0 ? (Math.abs(diferencia) / corte.saldoEsperado) * 100 : 0
...
export function obtenerSaldoAnterior(
  ...
  const cortesOrdenados = cortesHistoricos
    .filter((c) => c.banco === banco && c.estado === 'cerrado' && c.fecha < fecha)
  ...
  return cortesOrdenados[0]?.saldoFinal || 0
}
...
export function generarHistorialCortes(
  ...
  const cortesFiltrados = banco ? cortes.filter((c) => c.banco === banco) : cortes

  const cortesOrdenados = [...cortesFiltrados].sort((a, b) => b.fecha.getTime() - a.fecha.getTime())

  const cortesConDiscrepancia = cortesFiltrados.filter((c) => c.estado === 'discrepancia').length
...
export function generarCierreMensual(
  ...
  const saldoInicial = cortesOrdenados.length > 0 ? cortesOrdenados[0]?.saldoInicial || 0 : 0
  const saldoFinal =
    cortesOrdenados.length > 0 ? cortesOrdenados[cortesOrdenados.length - 1]?.saldoFinal || 0 : 0

  const totalIngresos = cortesOrdenados.reduce(
    (sum, c) => sum + c.totalIngresos + c.totalTransferenciasEntrada,
    0,
  )

  const totalGastos = cortesOrdenados.reduce(
    (sum, c) => sum + c.totalGastos + c.totalTransferenciasSalida,
    0,
  )

  const totalDiscrepancias = cortesOrdenados.reduce((sum, c) => sum + Math.abs(c.diferencia), 0)

  const diasConDiscrepancia = cortesOrdenados.filter((c) => c.estado === 'discrepancia').length
