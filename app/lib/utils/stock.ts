...
export function calcularNuevoStock(
  ...
  if (nuevoStock < 0) {
    ...
    throw new Error(
      `Stock no puede ser negativo. Actual: ${stockActual}, Movimiento: ${movimiento.cantidad}`,
    )
  }
...
export function valorizarInventario(productos: Producto[]): ValorizacionInventario {
  ...
  const margenPorcentaje = costoTotal > 0 ? ((precioVentaTotal - costoTotal) / costoTotal) * 100 : 0
 ...
 */
export function calcularCostoPromedioPonderado(movimientos: MovimientoStock[]): number {
  const entradas = movimientos.filter((m) => m.tipo === 'entrada' && m.costoUnitario !== undefined)
 ...
 */
export function generarTimelineMovimientos(movimientos: MovimientoStock[]): Array<{
  fecha: Date
  tipo: string
  cantidad: number
