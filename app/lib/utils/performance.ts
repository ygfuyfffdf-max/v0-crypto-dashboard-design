...
class PerformanceMonitor {
  ...
  measureBetween(name: string, startMark: string, endMark: string): void {
    ...
    try {
      ...
      if (measures.length > 0) {
        ...
        if (lastMeasure?.duration !== undefined) {
  ...
  getReport(): string {
    ...
    const report = this.metrics.map((m) => `${m.name}: ${m.duration.toFixed(2)}ms`).join('\n')
 ...
 * Throttle optimizado
 */
export function throttle<T extends (...args: Parameters<T>) => void>(fn: T, wait: number): T {
  let lastTime = 0
 ...
 */
export function batchUpdates<T>(callback: (items: T[]) => void, wait = 16): (item: T) => void {
  let batch: T[] = []
  let timeoutId: NodeJS.Timeout | null = null
