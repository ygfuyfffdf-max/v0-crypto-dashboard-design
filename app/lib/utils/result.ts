...
export class ValidationError extends AppError {
  ...
  constructor(errors: string[], context?: Record<string, unknown>) {
    super('VALIDATION_ERROR', `Datos inválidos: ${errors.join(', ')}`, { ...context, errors })
...
export class InsufficientStockError extends AppError {
  constructor(productoId: string, stockDisponible: number, cantidadSolicitada: number) {
    super(
      'INSUFFICIENT_STOCK',
...
export class InsufficientFundsError extends AppError {
  constructor(bancoId: string, saldoActual: number, montoRequerido: number) {
    super(
      'INSUFFICIENT_FUNDS',
...
export class DuplicateEntryError extends AppError {
  constructor(resource: string, field: string, value: string) {
    super('DUPLICATE_ENTRY', `Ya existe un ${resource} con ${field}: "${value}"`, {
      resource,
      field,
      value,
    })
 ...
 */
export function fromZodErrors(
  errors: { path: (string | number)[]; message: string }[],
): ValidationError {
  return new ValidationError(errors.map((e) => `${e.path.join('.')}: ${e.message}`))
}

/**
 * Crea un error desde un error de base de datos (Drizzle/SQLite)
 */
export function fromDatabaseError(error: unknown): DatabaseError {
...
/**
 * @deprecated Usar fromDatabaseError - alias para compatibilidad
 */
export const fromFirebaseError = fromDatabaseError
