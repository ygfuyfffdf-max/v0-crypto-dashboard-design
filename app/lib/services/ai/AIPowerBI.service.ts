...
import { collection, db, getDocs } from '@/app/lib/firebase/config'
...
export class AIPowerBIService {
   ...
   */
  private async generateKPIs(type: DashboardType, filters: DashboardFilters): Promise<KPI[]> {
    const kpis: KPI[] = []
  ...
  private async getVentasKPIs(filters: DashboardFilters): Promise<KPI[]> {
    ...
    const ventas = snapshot.docs.map((doc) => doc.data() as Record<string, unknown>)

    const totalVentas = ventas.reduce((sum: number, v) => sum + (Number(v.total) || 0), 0)
    ...
    const ventasRecientes = ventas.filter((v) => {
      const fechaVal = v.fecha as { toDate?: () => Date } | string | undefined
      const fecha =
        typeof fechaVal === 'object' && fechaVal?.toDate
          ? fechaVal.toDate()
          : new Date(String(fechaVal || ''))
    ...
    const totalReciente = ventasRecientes.reduce(
      (sum: number, v) => sum + (Number(v.total) || 0),
      0,
    )
    const totalAnterior = ventasAnteriores.reduce(
      (sum: number, v) => sum + (Number(v.total) || 0),
      0,
    )

    const cambio = totalAnterior > 0 ? ((totalReciente - totalAnterior) / totalAnterior) * 100 : 0
  ...
  private async getInventarioKPIs(filters: DashboardFilters): Promise<KPI[]> {
    ...
    const stockBajo = productos.filter((p) => Number(p.stockActual) <= Number(p.stockMinimo)).length

    const valorInventario = productos.reduce(
      (sum, p) => sum + (Number(p.stockActual) || 0) * (Number(p.costo) || 0),
      0,
    )
  ...
  private async getFinancieroKPIs(filters: DashboardFilters): Promise<KPI[]> {
    ...
    const capitalTotal = bancos.reduce(
      (sum, b) => sum + (Number(b.capitalActual) || Number(b.saldo) || 0),
      0,
    )

    const bancosActivos = bancos.filter(
      (b) => (Number(b.capitalActual) || Number(b.saldo) || 0) > 0,
    ).length
   ...
   */
  private generateRecommendations(insights: Insight[], type: DashboardType): Recommendation[] {
  ...
  private generateSparklineData(items: unknown[], days: number): number[] {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const dayTotal = (items as ItemWithFecha[]).reduce((sum, item) => {
        ...
        if (
          item.fecha &&
          typeof item.fecha === 'object' &&
          'toDate' in item.fecha &&
