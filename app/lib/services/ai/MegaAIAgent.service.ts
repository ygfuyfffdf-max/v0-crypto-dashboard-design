...
export class MegaAIAgentService {
  ...
  async sendMessage(request: AIRequest): Promise<AIResponse> {
    try {
      ...
      await this.learningService.trackActivity(request.userId, 'chat_message', 'general', {
      ...
      return {
        ...
        suggestions: ['Ver ventas del día', 'Mostrar estado de bancos', 'Generar reporte'],
      }
    }
  }
  ...
  private analyzeIntent(message: string): DetectedIntent {
    ...
    // Detectar ayuda
    if (
      this.matchesPatterns(lowerMessage, ['ayuda', 'help', 'qué puedes', 'comandos', 'opciones'])
    ) {
      return {
        type: 'help',
        confidence: 0.95,
  ...
  private detectEntity(message: string): string {
    ...
    if (message.includes('banco') || message.includes('capital') || message.includes('saldo')) {
      return 'bancos'
    }
  ...
  private async handleQueryData(intent: DetectedIntent, request: AIRequest): Promise<AIResponse> {
    ...
    try {
      ...
      await this.learningService.trackActivity(request.userId, 'query_collection', collectionName, {
        timeframe,
      })
  ...
  private calculateStats(data: unknown[], collectionName: string): Record<string, unknown> {
    ...
    switch (collectionName) {
      ...
      case 'productos':
        const stockBajo = items.filter((p) => Number(p.stockActual) <= Number(p.stockMinimo)).length
        return {
          count: items.length,
          stockBajo,
          tipo: 'productos',
      ...
      case 'bancos':
        const totalCapital = items.reduce(
          (sum, b) => sum + (Number(b.capitalActual) || Number(b.saldo) || 0),
          0,
        )
        return {
          count: items.length,
  ...
  private generateSuggestions(entity: string): string[] {
    const suggestions: Record<string, string[]> = {
      ...
      clientes: ['Ver clientes activos', 'Clientes con adeudo', 'Agregar nuevo cliente'],
      productos: [
  ...
  private async handleCreateRecord(
    ...
    if (!collectionName) {
      return {
        type: 'text',
        message:
          '¿Qué tipo de registro deseas crear? Puedo ayudarte con:\n\n' +
          '• Nueva venta\n' +
          '• Nuevo cliente\n' +
    ...
    return {
      type: 'action',
      message:
        `✨ Perfecto, vamos a crear un nuevo registro en **${collectionName}**.\n\n` +
        `${
  ...
  private async handleGenerateReport(
    ...
    const dashboard = await this.powerBIService.generateDashboard(request.userId, entity, {
      timeframe,
    })
  ...
  private async handleAnalyze(intent: DetectedIntent, request: AIRequest): Promise<AIResponse> {
    ...
    if (insights.length > 0) {
      insightsText =
        ...
        insights
          .map(
            (i: { type: string; title: string; description: string }) =>
    ...
    if (recommendations.length > 0) {
      recommendationsText =
        '\n\n**📋 Recomendaciones:**\n' +
        recommendations
          .map((r: { priority: string; title: string }, i: number) => `${i + 1}. ${r.title}`)
          .join('\n')
    }
  ...
  private async handleConversation(request: AIRequest): Promise<AIResponse> {
    ...
    if (greetings.some((g) => lowerMessage.includes(g))) {
      return {
        ...
        suggestions: ['Ver ventas del día', 'Estado de bancos', '¿Qué puedes hacer?'],
      }
    }
    ...
    if (thanks.some((t) => lowerMessage.includes(t))) {
      return {
        ...
        suggestions: ['No, eso es todo', 'Sí, otra consulta', 'Ver resumen del día'],
      }
    }
    ...
    return {
      type: 'text',
      message:
        '🤔 No estoy seguro de entender tu solicitud. ¿Puedes ser más específico?\n\n' +
        'Prueba con algo como:\n' +
        '• "Mostrar ventas del día"\n' +
        '• "Generar reporte de clientes"\n' +
        '• "Ir al panel de bancos"',
      suggestions: ['Ver ventas', 'Estado de bancos', '¿Qué puedes hacer?'],
    }
  }
  ...
  async createRecord(
    ...
    await this.learningService.trackActivity(this.userId, 'create_record', collectionName, {
      recordId: docRef.id,
    })
