...
export class AIScheduledReportsService {
  ...
  async createScheduledReport(
    config: Omit<ScheduledReport, 'id' | 'active' | 'createdAt' | 'updatedAt'>,
  ): Promise<string> {
    const cronPattern =
      config.cronPattern || CRON_PATTERNS[config.recurrence] || CRON_PATTERNS.weekly
  ...
  async executeScheduledReport(reportId: string): Promise<GeneratedReport> {
    ...
    const reportDoc = await getDocs(
      query(collection(db!, this.collectionName), where('__name__', '==', reportId)),
    )
  ...
  private async generateInventarioReport(filters?: ReportFilters): Promise<ReportData> {
    ...
    const stockBajo = productos.filter((p) => Number(p.stockActual) <= Number(p.stockMinimo))
