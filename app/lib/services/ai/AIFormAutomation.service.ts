...
export class AIFormAutomationService {
  ...
  async learnFromSubmission(
    ...
    const q = query(patternsRef, where('userId', '==', userId), where('formType', '==', formType))
  ...
  private async getClienteRelatedSuggestions(clienteId: string): Promise<FormSuggestion[]> {
    ...
    try {
      ...
      if (!snapshot.empty) {
        ...
        const topMetodoPago = Object.entries(metodoPagoCounts).sort((a, b) => b[1] - a[1])[0]
      ...
      logger.warn('Error obteniendo sugerencias de cliente', {
        context: 'AIFormAutomationService',
        data: error,
      })
    }
