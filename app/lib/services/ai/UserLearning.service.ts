...
export class UserLearningService {
  ...
  private async flushActivities(): Promise<void> {
    ...
    try {
      ...
      logger.error('Error flushing activities', error, { context: 'UserLearningService' })
    }
  }
   ...
   */
  private async updateUserPatterns(userId: string, activities: UserActivity[]): Promise<void> {
    try {
  ...
  async getUserProfile(userId: string): Promise<UserProfile | null> {
    try {
      ...
      logger.error('Error getting user profile', error, { context: 'UserLearningService', userId })
      return null
    }
  }
  ...
  private countSessions(activities: UserActivity[]): number {
    const uniqueSessions = new Set(activities.map((a) => a.sessionId).filter(Boolean))
  ...
  private async getPreferences(userId: string): Promise<UserPreferences> {
    try {
      ...
      logger.error('Error getting preferences', error, { context: 'UserLearningService', userId })
    }
  ...
  async updatePreferences(userId: string, updates: Partial<UserPreferences>): Promise<boolean> {
    try {
      ...
      logger.error('Error updating preferences', error, { context: 'UserLearningService', userId })
      return false
    }
  }
  ...
  async generateLearningInsights(userId: string): Promise<LearningInsight[]> {
    ...
    if (profile.analytics.engagementScore < 30) {
      insights.push({
        ...
        description:
          'Hay muchas funcionalidades que aún no has explorado. ¿Te gustaría un tour guiado?',
        confidence: 0.8,
        actionable: true,
  ...
  destroy(): void {
    ...
    this.flushActivities().catch((err) => logger.error('Error in destroy flush', err))
  }
}
