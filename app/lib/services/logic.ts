...
export function calcularDistribucionVenta(
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // FÓRMULAS GYA SAGRADAS — LÓGICA EXACTA CHRONOS 2026
  // ═══════════════════════════════════════════════════════════════════════════
  // precioTotalVenta = precioVenta × cantidad (lo que PAGA el cliente)
  // El flete es un COSTO INTERNO, NO se cobra adicional al cliente
  //
  // DISTRIBUCIÓN A 3 BANCOS (del dinero que paga el cliente):
  // - Bóveda Monte = precioCompra × cantidad (COSTO del producto)
  // - Fletes = precioFlete × cantidad (COSTO de transporte INTERNO)
  // - Utilidades = (precioVenta - precioCompra - precioFlete) × cantidad (GANANCIA NETA)
  //
  // EJEMPLO: 10 unidades, precioVenta=$10,000, precioCompra=$6,300, flete=$500
  // - precioTotalVenta = 10,000 × 10 = $100,000 (COBRADO AL CLIENTE)
  // - Bóveda Monte = 6,300 × 10 = $63,000
  // - Fletes = 500 × 10 = $5,000 (costo interno de transporte)
  // - Utilidades = (10,000 - 6,300 - 500) × 10 = $32,000
  // - Total distribuido = 63,000 + 5,000 + 32,000 = $100,000 ✓
  //
  // CONTABILIDAD: El flete se RESTA de las utilidades (es un costo interno).
  // El cliente NO paga extra por el flete, solo paga el precioVenta.
  // ═══════════════════════════════════════════════════════════════════════════

  const precioTotalVenta = precioVentaUnidad * cantidad // Lo que PAGA el cliente
  const montoBovedaMonte = (precioCompraUnidad || 0) * cantidad // COSTO → Bóveda Monte
  const montoFletes = (precioFlete || 0) * cantidad // FLETE → Flete Sur (costo interno)
  const montoUtilidades =
    (precioVentaUnidad - (precioCompraUnidad || 0) - (precioFlete || 0)) * cantidad // GANANCIA → Utilidades
  ...
  return {
    ...
    total: precioTotalVenta, // Total COBRADO al cliente (SIN flete adicional)
  }
}
 ...
 */
export function calcularAbono(venta: Venta, montoAbono: number): ResultadoAbono {
  ...
  const utilidadesOriginal =
    venta.distribucionBancos?.utilidades || venta.ganancia || venta.utilidad || 0
...
export function calcularTransferencia(
  ...
  if (monto > bancoOrigen.capitalActual) {
    throw new Error(
      `Fondos insuficientes. Disponible: ${bancoOrigen.capitalActual.toLocaleString()}`,
    )
  }
 ...
 */
export function recalcularBanco<T extends Banco>(banco: T, movimientos: Movimiento[]): T {
 ...
 */
export function recalcularCliente(cliente: Cliente, ventas: Venta[]): { deudaTotal: number } {
  const deudaTotal = ventas
    .filter(
      (v) =>
        v.clienteId === cliente.id && v.estadoPago !== 'completo' && v.estadoPago !== 'cancelado',
    )
 ...
 */
export function calcularDiasLiquidez(capitalTotal: number, gastoPromedioDiario: number): number {
 ...
 */
export function calcularMargenNeto(utilidades: number, ventasTotales: number): number {
...
export function calcularSaludFinanciera(
  ...
  const puntajeLiquidez = Math.min(100, (liquidezDias / 120) * 100) * 0.3
  const puntajeMargen = Math.min(100, (margenNeto / 50) * 100) * 0.25
  const puntajeROCE = Math.min(100, (roce / 60) * 100) * 0.25
  const puntajeRotacion = Math.min(100, (rotacionCapital / 8) * 100) * 0.2
