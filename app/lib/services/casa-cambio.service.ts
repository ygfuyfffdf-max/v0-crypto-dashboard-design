...
class CasaCambioService {
  ...
  async getCurrentRate(): Promise<number> {
    try {
      const headers = this.banxicoToken ? { 'Bmx-Token': this.banxicoToken } : {}
  ...
  async getHistoricalRates(days: number = 30): Promise<ExchangeRate[]> {
    try {
      ...
      return response.data.bmx.series[0].datos
        .map((item) => ({
          date: item.fecha,
   ...
   */
  calculateTechnicalIndicators(historicalRates: ExchangeRate[]): TechnicalIndicators {
  ...
  private calculateBollingerBands(
    ...
    const middle = recentPrices.reduce((a, b) => a + b, 0) / recentPrices.length

    const variance =
      recentPrices.reduce((sum, val) => sum + Math.pow(val - middle, 2), 0) / recentPrices.length
    const stdDev = Math.sqrt(variance)
