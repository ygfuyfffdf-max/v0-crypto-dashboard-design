    ...
    const resultado = await runTransaction(db!, async (transaction) => {
      ...
      if (venta.montoPagado > 0) {
        ...
        const movimientosData: Array<{ bancoId: BancoId; monto: number; concepto: string }> = [
          {
            bancoId: bancoCosto,
            monto: distribucion[bancoCosto],
            concepto: 'Costo de producto vendido',
          },
 ...
 */
export async function procesarOrdenCompraAtomica(
  ...
  try {
    const resultado = await runTransaction(db!, async (transaction) => {
      ...
      if (!entidadSnap.exists()) {
        throw new Error(
          `${abono.entidadTipo === 'cliente' ? 'Cliente' : 'Distribuidor'} no encontrado`,
        )
      }
