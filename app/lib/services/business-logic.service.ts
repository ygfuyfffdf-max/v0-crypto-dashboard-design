...
export async function registrarVenta(input: NuevaVentaInput): Promise<string> {
  ...
  try {
    await runTransaction(firestore, async (transaction) => {
      ...
      if (input.ocRelacionada) {
        ...
        if (ocDoc.exists()) {
          ...
          if (input.cantidad > stockDisponible) {
            throw new Error(
              `Stock insuficiente. Disponible: ${stockDisponible}, Solicitado: ${input.cantidad}`,
            )
          }
        }
      }
      ...
      const estadoPago =
        montoPagado >= distribucion.ingresoVenta
          ? 'completo'
          : montoPagado > 0
            ? 'parcial'
            : 'pendiente'
      ...
      if (clienteDoc.exists()) {
        ...
        transaction.update(clienteRef, {
          ...
          pendiente:
            (clienteData.deuda ?? 0) + montoRestante - ((clienteData.abonos ?? 0) + montoPagado),
      ...
      transaction.set(movBovedaRef, {
        ...
        montoPagado:
          montoPagado > 0
            ? (distribucion.montoBovedaMonte * montoPagado) / distribucion.ingresoVenta
            : 0,
      ...
      if (distribucion.montoFletes > 0) {
        ...
        transaction.set(movFletesRef, {
          ...
          montoPagado:
            montoPagado > 0
              ? (distribucion.montoFletes * montoPagado) / distribucion.ingresoVenta
              : 0,
      ...
      transaction.set(movUtilidadesRef, {
        ...
        montoPagado:
          montoPagado > 0
            ? (distribucion.montoUtilidades * montoPagado) / distribucion.ingresoVenta
            : 0,
    ...
    await runTransaction(firestore, async (transaction) => {
      ...
      if (input.ordenCompraRelacionada) {
        ...
        if (ocDoc.exists()) {
          ...
          const nuevoEstadoOC =
            nuevaDeudaOC === 0 ? 'pagado' : input.monto > 0 ? 'parcial' : ocData.estado
...
export async function registrarOrdenCompra(
  ...
  try {
    await runTransaction(firestore, async (transaction) => {
      ...
      const estado = deuda === 0 ? 'pagado' : pagoInicial > 0 ? 'parcial' : 'pendiente'
      ...
      transaction.set(ocRef, {
        ...
        bancoOrigen,
        estado,
        keywords: [ocId.toLowerCase(), distribuidorNombre.toLowerCase()],
        notas,
        createdAt: Timestamp.now(),
        updatedAt: Timestamp.now(),
      })
