...
export class VoiceRecognitionService {
  ...
  private initialize(): void {
    ...
    if (typeof window === 'undefined') {
      logger.warn('VoiceRecognitionService: No window object', {
  ...
  startListening(): boolean {
    if (!this.recognition) {
      logger.error('Speech recognition not available', new Error('Not supported'), {
        context: 'VoiceRecognitionService',
      })
      return false
    }
  ...
  speak(
    text: string,
    options?: Partial<{ rate: number; pitch: number; volume: number; voice: string }>,
  ): Promise<void> {
