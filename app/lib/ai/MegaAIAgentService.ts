...
💡 **Tip:** Puedes hablarme naturalmente, entiendo español y el contexto de tu negocio.`
   ...
   */
  private generateVentasResponse(
    data?: Record<string, unknown>,
    entities?: Record<string, unknown>,
  ): string {
    if (!data?.ventas) {
    ...
    const total = (ventas as { precioTotalVenta: number }[]).reduce(
      (sum, v) => sum + v.precioTotalVenta,
      0,
    )
...
¿Deseas ver más detalles o aplicar algún filtro?`
   ...
   */
  private generateClientesResponse(
    data?: Record<string, unknown>,
    entities?: Record<string, unknown>,
  ): string {
    if (!data?.clientes) {
...
¿Necesitas ver la lista de clientes morosos o registrar un abono?`
   ...
   */
  private generateDistribuidoresResponse(
    data?: Record<string, unknown>,
    entities?: Record<string, unknown>,
  ): string {
...
¿Deseas ver el detalle de algún distribuidor o registrar un pago?`
   ...
   */
  private generateBancosResponse(
    data?: Record<string, unknown>,
    entities?: Record<string, unknown>,
  ): string {
    if (!data?.bancos) {
...
¿Deseas ver movimientos o hacer una transferencia?`
   ...
   */
  private generateAlmacenResponse(
    data?: Record<string, unknown>,
    entities?: Record<string, unknown>,
  ): string {
    if (!data?.productos) {
...
¿Deseas ver los productos con stock crítico?`
   ...
   */
  private generateOrdenesResponse(
    data?: Record<string, unknown>,
    entities?: Record<string, unknown>,
  ): string {
    if (!data?.ordenes) {
