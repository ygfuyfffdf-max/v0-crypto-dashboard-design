...
export type ElevenLabsVoice =
  ...
  // 🇲🇽 NUEVAS VOCES ESPAÑOL NATIVO (2025)
  | 'carlos' // Profesional mexicano masculina
  | 'valentina' // Ejecutiva mexicana femenina
  | 'mateo' // Casual mexicano joven
  | 'sofia' // Amigable mexicana
...
export type ElevenLabsState = 'idle' | 'loading' | 'playing' | 'error'
...
export const VOICE_IDS: Record<ElevenLabsVoice, string> = {
  ...
  // 🇲🇽 VOCES ESPAÑOL NATIVO (IDs placeholder - reemplazar con IDs reales de tu cuenta)
  carlos: 'JBFqnCBsd6RMkjVDRZzb', // Spanish male professional
  valentina: 'EXAVITQu4vr4xnSDxMaL', // Spanish female executive
  mateo: 'TxGEqnHWrfWFTfGW9XjX', // Spanish male casual
  sofia: 'MF3mGyEYCl7XYWbV9V6O', // Spanish female friendly
}

// 🎭 FINANCE-SPECIFIC VOICE SETTINGS
const VOICE_SETTINGS_BY_EMOTION: Record<
  VoiceEmotion,
  { stability: number; similarity: number; style: number }
> = {
  neutral: { stability: 0.5, similarity: 0.75, style: 0.0 },
  excited: { stability: 0.3, similarity: 0.8, style: 0.7 }, // Para buenas noticias de ventas
  calm: { stability: 0.8, similarity: 0.6, style: 0.2 }, // Para reportes
  concerned: { stability: 0.6, similarity: 0.7, style: 0.4 }, // Para alertas de stock bajo
  professional: { stability: 0.7, similarity: 0.75, style: 0.3 }, // Default para finanzas
}

// 🔊 FINANCE CONTEXT VOICE PRESETS
export const FINANCE_VOICE_PRESETS = {
  salesReport: { voice: 'carlos' as ElevenLabsVoice, emotion: 'professional' as VoiceEmotion },
  goodNews: { voice: 'sofia' as ElevenLabsVoice, emotion: 'excited' as VoiceEmotion },
  alert: { voice: 'valentina' as ElevenLabsVoice, emotion: 'concerned' as VoiceEmotion },
  summary: { voice: 'mateo' as ElevenLabsVoice, emotion: 'calm' as VoiceEmotion },
}
...
export class ElevenLabsTTSClient {
  ...
  async speak(text: string, emotion: VoiceEmotion = 'neutral'): Promise<void> {
    ...
    try {
      ...
      // Fallback a Web Speech API
      logger.warn('[ElevenLabsTTS] Usando Web Speech API como fallback', {
    ...
    return [
      {
        id: 'rachel',
        name: 'Rachel',
        description: 'Profesional femenina',
  ...
  private speakWithWebSpeech(text: string): void {
    if (typeof window === 'undefined' || !window.speechSynthesis) {
      logger.error(
        '[ElevenLabsTTS] Web Speech API no disponible',
        new Error('No Speech Synthesis'),
        { context: 'ElevenLabsTTS' },
      )
      return
    }
  ...
  private handleError(error: Error): void {
    ...
    if (typeof window !== 'undefined') {
      const win = window as unknown as {
        Sentry?: { captureException: (e: Error, opts?: Record<string, unknown>) => void }
      }
      if (win.Sentry) {
...
export function detectEmotion(text: string): VoiceEmotion {
  ...
  const patterns: Record<VoiceEmotion, RegExp[]> = {
    ...
    calm: [/tranquilo|calma|listo|preparado|completado/, /sin problemas|todo bien|correcto/],
    professional: [/informe|reporte|análisis|resumen|estadísticas/, /según|conforme|de acuerdo/],
    neutral: [],
  }
