...
export function useVoice(config: UseVoiceConfig = {}): UseVoiceReturn {
  ...
  const handleSttError = useCallback(
    (err: Error) => {
      ...
      if (config.useFallback !== false) {
        ...
        webSpeechRef.current = new WebSpeechFallback(handleTranscript, (fallbackErr) => {
  ...
  const speak = useCallback(
    async (text: string, emotion?: VoiceEmotion) => {
      if (!ttsClientRef.current) {
        logger.error('[useVoice] Cliente TTS no inicializado', new Error('TTS not initialized'), {
          context: 'useVoice',
        })
        return
      }
      ...
      try {
        ...
        const finalEmotion =
          emotion || (config.autoDetectEmotion !== false ? detectEmotion(text) : 'neutral')
  ...
  const askAndListen = useCallback(
    async (prompt: string, emotion?: VoiceEmotion): Promise<string> => {
      // 1. Reproducir prompt
