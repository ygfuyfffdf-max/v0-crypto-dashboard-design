...
export interface DeepgramConfig {
  apiKey?: string
  language?: string
  model?: 'nova-3' | 'nova-2' | 'nova' | 'base' // Nova-3 para latencia <100ms
  smartFormat?: boolean
  punctuate?: boolean
  diarize?: boolean
  interimResults?: boolean
  chunkInterval?: number // Intervalo de envío en ms (default 500 para Nova-3)
  sentiment?: boolean // Análisis de sentimiento (Nova-3)
  topics?: boolean // Detección de temas (Nova-3)
  onTranscript?: (text: string, isFinal: boolean) => void
  onSentiment?: (sentiment: 'positive' | 'negative' | 'neutral') => void
...
const DEFAULT_CHUNK_INTERVAL = 500 // 500ms para Nova-3 (antes era 1000)

// =====================================================================
// DEEPGRAM STT CLIENT (REST Polling Mode) - UPGRADED TO NOVA-3
// =====================================================================
...
export class DeepgramSTTClient {
  ...
  constructor(config: DeepgramConfig = {}) {
    this.config = {
      language: 'es-MX',
      model: 'nova-3', // Upgraded from nova-2 for <100ms latency
      smartFormat: true,
      punctuate: true,
      diarize: true, // Now enabled by default
      interimResults: true,
      chunkInterval: DEFAULT_CHUNK_INTERVAL,
      sentiment: true, // Nova-3 feature
      topics: false,
      ...config,
    }
  }
  ...
  private handleReconnect(): void {
    ...
    logger.warn(
      `[DeepgramSTT] Reintentando... (${this.reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS})`,
      {
        context: 'DeepgramSTT',
        data: { delay },
      },
