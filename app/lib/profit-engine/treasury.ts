...
export function calculateInventoryHealthScore(
  ...
  const calculateBalanceScore = (utilization: number): number => {
    if (utilization >= 0.3 && utilization <= 0.7) {
      ...
      return 0.3 // Crítico
    } else if (utilization < 0.2 || utilization > 0.8) {
...
export function calculateOptimalBoardPrice(
  ...
  if (mode === 'sell') {
    ...
    if (inventoryPercent > thresholds.HIGH_INVENTORY_PERCENT) {
      adjustment = -0.1 // 10 centavos abajo
    ...
    if (inventoryPercent < thresholds.LOW_INVENTORY_PERCENT) {
      adjustment = 0.1 // 10 centavos arriba para atraer
...
export function generateTreasuryAlerts(
  ...
  const totalUsdValue =
    position.physical_vault_usd + position.usa_bank_usd + position.digital_wallets_usdt
  ...
  if (usdtPercent < 0.15 && totalUsdValue > 50000) {
    alerts.push({
      ...
      message:
        'Solo el 15% de tu capital está en USDT. Considera convertir para aprovechar primas del mercado P2P.',
...
export async function recordTreasuryMovement(
  ...
  try {
    ...
    const field =
      `${movement.location}_${movement.currency.toLowerCase()}` as keyof TreasuryPosition
    const currentBalance = (currentPos[field] as number) || 0
    const newBalance =
      movement.type === 'in' ? currentBalance + movement.amount : currentBalance - movement.amount
