 ...
 */
export function useRealtimeVentas(options?: { limit?: number; startDate?: Date; endDate?: Date }) {
 ...
 */
export function useRealtimeClientes(options?: { limit?: number; conDeuda?: boolean }) {
...
export function useRealtimeBancos(bancoId?: string) {
  const constraints: QueryConstraint[] = [orderBy('fecha', 'desc'), limit(50)]
...
export function useRealtimeStats() {
  ...
  useEffect(() => {
    ...
    // Stock bajo
    const stockBajoQuery = query(collection(db!, 'almacen'), where('cantidad', '<', 10))
    unsubscribers.push(
...
export function useRealtimeDashboard() {
  ...
  return {
    ...
    stats,
    loading:
      ventas.loading || ordenes.loading || clientes.loading || saldos.loading || statsLoading,
