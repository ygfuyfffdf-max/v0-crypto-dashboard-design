  ...
  const setVariable = useCallback((name: string, value: string | number | boolean) => {
    if (appRef.current) {
      try {
        ...
        logger.warn(`Error setting Spline variable ${name}`, { context: 'useSpline', data: error })
      }
    }
  }, [])
  ...
  const getVariable = useCallback((name: string): unknown => {
    if (appRef.current) {
      try {
        ...
        logger.warn(`Error getting Spline variable ${name}`, { context: 'useSpline', data: error })
        return undefined
      }
    }
    return undefined
  ...
  const triggerEvent = useCallback((objectName: string, eventName: string) => {
    if (appRef.current) {
      try {
        ...
        logger.warn(`Error triggering event ${eventName} on ${objectName}`, {
          context: 'useSpline',
          data: error,
        })
      }
    ...
    (cameraName: string) => {
      if (appRef.current) {
        try {
          ...
          logger.warn(`Error setting camera ${cameraName}`, { context: 'useSpline', data: error })
        }
      }
    },
    [setVariable],
...
export function useSplineOrb() {
  ...
  const setOrbEmotion = useCallback(
    (emotion: 'happy' | 'sad' | 'neutral' | 'excited' | 'curious') => {
      controls.setVariable('orb_emotion', emotion)
    },
    [controls],
  )

  const setOrbSize = useCallback(
...
export function useSplineRobot() {
  ...
  const setExpression = useCallback(
    (expression: 'happy' | 'thinking' | 'surprised' | 'neutral') => {
