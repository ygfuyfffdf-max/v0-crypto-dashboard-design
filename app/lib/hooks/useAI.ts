...
export function useAI(userId: string = 'default-user'): UseAIResult {
  ...
  const sendMessage = useCallback(
    async (message: string, context?: { userId?: string }): Promise<AIResponse | null> => {
        ...
        insights: [
          {
            ...
            description: 'Incremento del 18% en ventas',
          },
          { type: 'info', title: 'Predicción', description: 'Alcanzar 76K en ventas próximo mes' },
          {
            type: 'warning',
            title: 'Stock Bajo',
            description: 'Reponer inventario en 3 productos',
          },
        ],
      }
    } catch (error) {
