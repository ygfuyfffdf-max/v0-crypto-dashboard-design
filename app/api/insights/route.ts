...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    switch (type) {
      case 'analyze': {
        if (!metrics) {
          return NextResponse.json({ error: 'Métricas requeridas' }, { status: 400 })
        }
      ...
      case 'report': {
        if (!reportData) {
          return NextResponse.json({ error: 'Datos de reporte requeridos' }, { status: 400 })
        }

        const crecimiento = (
          ((reportData.ventas - reportData.ventasAnterior) / reportData.ventasAnterior) *
          100
        ).toFixed(1)
