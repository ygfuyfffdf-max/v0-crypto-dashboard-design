...
async function transcribeMock(text?: string): Promise<TranscribeResponse> {
  ...
  return {
    text:
      mockResponses[Math.floor(Math.random() * mockResponses.length)] || 'Comando de voz recibido',
    confidence: 0.95,
    language: 'es-MX',
    provider: 'mock',
    duration: 2.5,
...
async function transcribeWhisper(
  ...
  formData.append('language', language?.split('-')[0] || 'es')
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { audio, format = 'wav', language = 'es-MX', provider: preferredProvider } = body

    if (!audio) {
      return NextResponse.json({ error: 'Audio es requerido (base64)' }, { status: 400 })
    }
