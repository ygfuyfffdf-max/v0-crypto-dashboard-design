...
// Tipo para proveedores de IA del SDK
type AIProvider =
...
interface ProviderConfig {
  ...
  provider: AIProvider
  modelId: string
  useDirectFetch?: boolean // Para GitHub Models que no soporta el nuevo endpoint
}

const getAIProvider = (): ProviderConfig => {
  // OpenAI (preferido - soporta el SDK v5 completamente)
  ...
  // GitHub Models (usa fetch directo por compatibilidad)
...
const ventasSchema = z.object({
  periodo: z
    .enum(['hoy', 'ayer', 'semana', 'mes', 'año', 'custom'])
    .optional()
...
const reporteSchema = z.object({
  tipo: z
    .enum(['ventas', 'financiero', 'inventario', 'clientes', 'distribuidores'])
    .describe('Tipo de reporte'),
...
const analisisSchema = z.object({
  area: z
    .enum(['ventas', 'inventario', 'finanzas', 'clientes', 'general'])
    .describe('Área a analizar'),
...
const systemTools = {
  obtenerVentas: tool({
    description:
      'Obtiene información de ventas del sistema. Puede filtrar por fecha, cliente, distribuidor, etc.',
  ...
  obtenerClientes: tool({
    description:
      'Obtiene información de clientes. Puede buscar por nombre, teléfono o filtrar por estado.',
...
Solo pregúntame lo que necesites.`
  ...
  if (/ir\s*a|abrir|muestra|ve\s*a/i.test(lowerMessage)) {
    const panelMatch = lowerMessage.match(
      /(dashboard|ventas?|clientes?|distribuidores?|bancos?|almacén|reportes?)/i,
    )
    if (panelMatch) {
...
// ============================================================================
// GITHUB MODELS DIRECT FETCH (Compatible con chat/completions)
// ============================================================================
...
export async function POST(request: NextRequest) {
  try {
    ...
    // GitHub Models - usar fetch directo para compatibilidad
    if (providerConfig.useDirectFetch && providerConfig.type === 'github') {
      logger.info('Using GitHub Models direct fetch', {
        context: 'ConversationalAI',
        data: { userId, model: providerConfig.modelId },
      })

      const stream = await fetchGitHubModels(messages, SYSTEM_PROMPT)
      return new Response(stream, {
        headers: {
          'Content-Type': 'text/plain; charset=utf-8',
          'Cache-Control': 'no-cache',
    ...
    // Otros proveedores - usar SDK de AI
