...
const log = {
  title: (msg: string) =>
    console.log(
      `\n${c.bold}${c.violet}${"═".repeat(70)}${c.reset}\n${c.bold}${c.white}  ${msg}${c.reset}\n${c.violet}${"═".repeat(70)}${c.reset}`
    ),
  ...
  data: (label: string, value: unknown) =>
    console.log(
      `${c.magenta}    ${label}:${c.reset} ${typeof value === "object" ? JSON.stringify(value) : value}`
    ),
  error: (msg: string) => console.log(`${c.red}  ✗ ERROR: ${msg}${c.reset}`),
  money: (label: string, amount: number) =>
    console.log(`${c.green}    💰 ${label}: $${amount.toLocaleString()}${c.reset}`),
...
async function crearOrdenCompra(distribuidorId: string) {
  ...
  const { producto, origen, cantidad, costoPorUnidad, precioFlete, pagoInicial } =
    TEST_DATA.ordenCompra
...
async function crearVenta(clienteId: string, productoId: string, ordenId: string) {
  ...
  log.success(
    `Bóveda Monte: $${capitalMonteAntes.toLocaleString()} → $${capitalMonteDespues.toLocaleString()} (+$${montoBovedaMonteReal.toLocaleString()})`
  )
  log.success(
    `Flete Sur: $${capitalFleteAntes.toLocaleString()} → $${capitalFleteDespues.toLocaleString()} (+$${montoFletesReal.toLocaleString()})`
  )
  log.success(
    `Utilidades: $${capitalUtilidadesAntes.toLocaleString()} → $${capitalUtilidadesDespues.toLocaleString()} (+$${montoUtilidadesReal.toLocaleString()})`
  )
...
async function pagarDistribuidor(distribuidorId: string, ordenId: string) {
  ...
  log.success(
    `Bóveda Monte: $${capitalAntes.toLocaleString()} → $${capitalDespues.toLocaleString()} (-$${montoPago.toLocaleString()})`
  )
}
...
async function registrarIngreso() {
  ...
  log.success(
    `Capital: $${capitalAntes.toLocaleString()} → $${capitalDespues.toLocaleString()} (+$${monto.toLocaleString()})`
  )
}
...
async function realizarTransferencia() {
  ...
  log.success(
    `${bancoOrigen}: $${capitalOrigenAntes.toLocaleString()} → $${capitalOrigenDespues.toLocaleString()}`
  )
  log.success(
    `${bancoDestino}: $${capitalDestinoAntes.toLocaleString()} → $${capitalDestinoDespues.toLocaleString()}`
  )
}
...
async function verificacionFinal() {
  ...
  bancosSnap.docs.forEach((doc) => {
    ...
    log.data(
      `  ${data.nombre || doc.id}`,
      `Capital: $${(data.capitalActual || 0).toLocaleString()}`
    )
  })
