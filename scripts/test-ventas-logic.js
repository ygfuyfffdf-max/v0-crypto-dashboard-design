...
function calcularDistribucion(ventaInput) {
  const { cantidad, precioVenta, precioCompra, precioFlete, montoPagado } = ventaInput
...
function printResultado(titulo, venta, resultado) {
  ...
  console.log(
    `  📈 Estado Pago: ${resultado.estadoPago.toUpperCase()} (${resultado.proporcionPagada})`
  )
  console.log()
  ...
  if (Math.abs(sumaDistribucion - resultado.totalVenta) < 0.01) {
    console.log(
      `  ✅ Distribución correcta: ${formatMoney(sumaDistribucion)} = ${formatMoney(resultado.totalVenta)}`
    )
  } else {
    console.log(
      `  ❌ ERROR: ${formatMoney(sumaDistribucion)} ≠ ${formatMoney(resultado.totalVenta)}`
    )
  }

  if (Math.abs(sumaCapital - venta.montoPagado) < 0.01) {
    console.log(
      `  ✅ Capital correcto: ${formatMoney(sumaCapital)} = ${formatMoney(venta.montoPagado)}`
    )
  } else {
