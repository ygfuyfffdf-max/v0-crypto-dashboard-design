...
const log = {
  title: (msg: string) =>
    console.log(
      `\n${colors.bright}${colors.violet}═══════════════════════════════════════════════════════════${colors.reset}`
    ),
...
`)
  ...
  for (const bancoId of bancosEsperados) {
    ...
    if (banco) {
      ...
      log.success(
        `${bancoData.nombre || bancoId}: Capital $${(bancoData.capitalActual || 0).toLocaleString()}`
      )
  ...
  if (distribuidores.count > 0) {
    distribuidores.data.forEach(
      (dist: { nombre?: string; deudaTotal?: number; totalPagado?: number }) => {
    ...
    log.warn(
      "No hay distribuidores - El formulario CreateDistribuidorModalPremium debe crear el primero"
    )
  }
  ...
  if (ordenes.count > 0) {
    ...
    log.warn(
      "No hay órdenes de compra - El formulario CreateOrdenCompraModalPremium debe crear la primera"
    )
  }
    ...
    ventas.data.forEach(
      (venta: {
        ...
        if (venta.distribucionBancos) {
          log.data(
            "  → Bóveda Monte",
            `$${(venta.distribucionBancos.bovedaMonte || 0).toLocaleString()}`
          )
          log.data("  → Fletes", `$${(venta.distribucionBancos.fletes || 0).toLocaleString()}`)
          log.data(
            "  → Utilidades",
            `$${(venta.distribucionBancos.utilidades || 0).toLocaleString()}`
          )
        }
      }
    )
...
`)
  ...
  bancos.forEach(
    (banco: { capitalActual?: number; historicoIngresos?: number; historicoGastos?: number }) => {
