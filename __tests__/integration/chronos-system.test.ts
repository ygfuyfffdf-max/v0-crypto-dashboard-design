 ...
 */
function actualizarCapitalBanco(banco: Banco, tipo: "ingreso" | "gasto", monto: number): Banco {
  if (tipo === "ingreso") {
    return {
      ...banco,
...
describe("🏦 LÓGICA DE DISTRIBUCIÓN GYA (Ganancia y Asignación)", () => {
  describe("Cálculo de Distribución a 3 Bancos", () => {
    it("✅ Distribución correcta: 10 unidades @ $10,000 venta, $6,300 costo, $500 flete", () => {
      const cantidad = 10
      const { totalVenta, montoBovedaMonte, montoFletes, montoUtilidades } =
    ...
    it("✅ Distribución con 1 unidad", () => {
      const { totalVenta, montoBovedaMonte, montoFletes, montoUtilidades } =
        calcularDistribucionGYA(1, PRECIO_VENTA_UNIDAD, PRECIO_COMPRA_UNIDAD, PRECIO_FLETE_UNIDAD)
    ...
    it("✅ Distribución con 100 unidades (operación grande)", () => {
      const { totalVenta, montoBovedaMonte, montoFletes, montoUtilidades } =
        calcularDistribucionGYA(100, PRECIO_VENTA_UNIDAD, PRECIO_COMPRA_UNIDAD, PRECIO_FLETE_UNIDAD)
    ...
    it("✅ Utilidades = Ganancia NETA (precio venta - costo - flete)", () => {
      ...
      const gananciaNetaEsperada =
        (PRECIO_VENTA_UNIDAD - PRECIO_COMPRA_UNIDAD - PRECIO_FLETE_UNIDAD) * cantidad
...
describe("💰 ACTUALIZACIÓN DE CAPITAL BANCARIO", () => {
  ...
  it("✅ Saldo = historicoIngresos - historicoGastos", () => {
    ...
    expect(despuesIngreso.saldo).toBe(
